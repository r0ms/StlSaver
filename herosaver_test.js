...
function init() {
	...
	(function () {
		// Decode skin weight that has been encoded for copy protection.
		// Intentionally duplicated local function to prevent finding it with code inspection.
		const modulo = function (x, y) {
			return x - y * Math.floor(x / y);
		}
		const decodeSkinWeight = function (value) {
			value = Math.abs(modulo(value + 1, 2) - 1);
			return value;
		};
		...
		character_area = jQuery("nav").first();
		character_area.css({
			"display": "flex",
...
inject_script("//code.jquery.com/jquery-3.3.1.min.js", function () {
	inject_script("//cdnjs.cloudflare.com/ajax/libs/three.js/r124/three.min.js", function () {
